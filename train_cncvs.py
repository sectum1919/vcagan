...
from src.data.vid_aud_cncvs import CNCVSMultiDataset, collater
...
def train_net(args):
    ...
    args.checkpoint_dir = os.path.join(
        args.checkpoint_dir,
        f'{args.dataset}_{args.subject}_{args.sample_rate}_{args.filter_length}_{args.hop_length}',
    )
    os.makedirs(args.checkpoint_dir, exist_ok=True)

    train_data = CNCVSMultiDataset(
    ...
    d_params = [{
        'params': dis1.parameters()
    }, {
        'params': dis2.parameters()
    }, {
        'params': dis3.parameters()
    }, {
    ...
    _ = validate(v_front, gen, post, val_data, fast_validate=True)
    train(v_front,
          gen,
          post,
          dis1,
          ...
          scheduler=[g_scheduler, d_scheduler],
          args=args)


def train(v_front, gen, post, dis1, dis2, dis3, s_dis, train_data, val_data, epochs, optimizer, scheduler, args):
    best_val_stoi = 0
            ...
            if writer is not None:
                ...
                if i % 100 == 0:
                    ...
                    writer.add_image('train_spec/gen_log',
                                     ...
                                     step)
                    writer.add_image('train_spec/gt', train_data.plot_spectrogram_to_numpy(spec.detach().numpy()[0]),
                                     step)
                    writer.add_image('train_spec/gt_log',
                                     train_data.plot_spectrogram_to_numpy(torch.log(spec).detach().numpy()[0]), step)
                    writer.add_audio('train_aud/pred_mel', wav_pred, global_step=step, sample_rate=args.sample_rate)
                    writer.add_audio('train_aud/pred_spec', wav_spec, global_step=step, sample_rate=args.sample_rate)
                    writer.add_audio('train_aud/gt_mel', wav_gt, global_step=step, sample_rate=args.sample_rate)
                    writer.add_audio('train_aud/gt_wav',
                                     wav_tr[0].numpy(),
                                     global_step=step,
                                     sample_rate=args.sample_rate)

            if step % args.eval_step == 0:
                logs = validate(v_front, gen, post, val_data, epoch=epoch, writer=writer, fast_validate=True)
                ...
                torch.save(
                    {
                        ...
                        'dis1_state_dict': dis1_state_dict,
                        'dis2_state_dict': dis2_state_dict,
                        'dis3_state_dict': dis3_state_dict,
                        's_dis_state_dict': s_dis_state_dict
                    },
                ...
                if logs[1] > best_val_stoi:
                    ...
                    best_ckpt = os.path.join(
                            args.checkpoint_dir,
                            'Best_%04d_stoi_%.3f_estoi_%.3f_pesq_%.3f.ckpt' % (epoch, logs[1], logs[2], logs[3]))
                    torch.save(
                        {
                            'v_front_state_dict': v_state_dict,
                            'gen_state_dict': gen_state_dict,
                    ...
                    best_soft_file = os.path.join(args.checkpoint_dir, 'Best.ckpt')
                    if os.path.exists(best_soft_file):
                        os.remove(best_soft_file)
                    os.system(f"ln -s {os.path.realpath(best_ckpt)} {best_soft_file}")
...
def validate(v_front, gen, post, val_data, fast_validate=False, epoch=0, writer=None):
    with torch.no_grad():
        v_front.eval()
        gen.eval()
        post.eval()
        ...
        for i, batch in enumerate(dataloader):
            ...
            for _ in range(g3.size(0)):
                stoi_list.append(
                    stoi(wav_tr[_][:len(wav_pred[_])].numpy(), wav_pred[_], args.sample_rate, extended=False))
                stoi_spec_list.append(stoi(wav_tr[_][:len(wav_spec[_])], wav_spec[_], args.sample_rate,
                                           extended=False))
                estoi_list.append(
                    stoi(wav_tr[_][:len(wav_pred[_])].numpy(), wav_pred[_], args.sample_rate, extended=True))
                estoi_spec_list.append(stoi(wav_tr[_][:len(wav_spec[_])], wav_spec[_], args.sample_rate,
                                            extended=True))
                try:
                    pesq_list.append(
                        pesq(8000, librosa.resample(wav_tr[_][:len(wav_pred[_])].numpy(), args.sample_rate, 8000),
                             librosa.resample(wav_pred[_], args.sample_rate, 8000), 'nb'))
                except:
                    pass
                try:
                    pesq_spec_list.append(
                        pesq(8000, librosa.resample(wav_tr[_][:len(wav_spec[_])].numpy(), args.sample_rate, 8000),
                             librosa.resample(wav_spec[_], args.sample_rate, 8000), 'nb'))
                except:
                    continue
            ...
            if i in [int(required_iter // 3), int(2 * (required_iter // 3)), int(3 * (required_iter // 3))]:
                if writer is not None:
                    ...
                    writer.add_image('val_spec_%d/gen_log' % i,
                                     val_data.plot_spectrogram_to_numpy(torch.log(gs).cpu().detach().numpy()[0]),
                                     epoch)
                    writer.add_image('val_spec_%d/gt' % i,
                                     val_data.plot_spectrogram_to_numpy(spec.detach().numpy()[0]), epoch)
                    writer.add_image('val_spec_%d/gt_log' % i,
                                     val_data.plot_spectrogram_to_numpy(torch.log(spec).detach().numpy()[0]), epoch)

                    writer.add_audio('val_aud_%d/pred' % i,
                                     wav_pred[0],
                                     global_step=epoch,
                                     sample_rate=args.sample_rate)
                    writer.add_audio('val_aud_%d/mel' % i, wav_gt[0], global_step=epoch, sample_rate=args.sample_rate)
                    writer.add_audio('val_aud_%d/spec' % i,
                                     wav_spec[0],
                                     global_step=epoch,
                                     sample_rate=args.sample_rate)
                    writer.add_audio('val_aud_%d/gt' % i, wav_tr[0], global_step=epoch, sample_rate=args.sample_rate)
                    fig = plt.figure()
        ...
        return np.mean(np.array(val_loss)), np.mean(np.array(stoi_spec_list)), np.mean(
            np.array(estoi_spec_list)), np.mean(np.array(pesq_spec_list))
