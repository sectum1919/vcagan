...
class CNCVSMultiDataset(Dataset):
    ...
    def build_file_list(self, datapath, mode, subject):
        file_list = []
        with open(os.path.join(datapath, 'filelists', f'{subject}-{mode}.txt'), 'r') as f:
            file_list = [os.path.join(datapath, 'video', line.strip().split('_')[0], line.strip() + '.mp4') for line in f.readlines()]
        return file_list
    ...
    def __getitem__(self, idx):
        ...
        vid, _, info = torchvision.io.read_video(file_path, pts_unit='sec')  # vid.size() 41 112 224 3
        audio, info['audio_fps'] = librosa.load(file_path.replace('video', 'audio')[:-4] + '.wav',
                                                sr=self.sample_rate)
        ...
        if vid.size(0) < 5 or audio.size(1) < 5:
            ...
            audio = torch.zeros([1, int(self.sample_rate // self.fps)])
        ...
        if self.mode == 'test':
            return melspec, spec, vid, num_v_frames, audio.squeeze(0), num_a_frames, file_path.replace(
                os.path.join(self.data_dir, 'video/'), '')[:-4]
        else:
