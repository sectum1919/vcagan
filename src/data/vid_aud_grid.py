...
class MultiDataset(Dataset):
    ...
    def __init__(
        ...
        # self.stft = TacotronSTFT(filter_length=640,
        #                          hop_length=160,
        #                          win_length=640,
        #                          n_mel_channels=80,
        #                          sampling_rate=16000,
    ...
    def build_tensor(self, frames):
        ...
        transform = transforms.Compose([
            ...
            Crop(crop),
            transforms.Resize([112, 112]), augmentations1,
    ...
    def __getitem__(self, idx):
        ...
        audio, info['audio_fps'] = librosa.load(file_path.replace('video', 'audio')[:-4] + '.flac',
                                                sr=self.sample_rate)
        ...
        if vid.size(0) < 5 or audio.size(1) < 5:
            ...
            audio = torch.zeros([1, self.sample_rate // 25])
...
class MultiDatasetPreprocess(MultiDataset):

    def __init__(self, *args, **kwargs):
        super(MultiDatasetPreprocess, self).__init__(*args, **kwargs)
    ...
    def __getitem__(self, idx):
        ...
        melspec = np.load(file_path.replace('video', 'audio')[:-4] + '.npy')
        spec = np.load(file_path.replace('video', 'audio')[:-4] + '_spec.npy')
